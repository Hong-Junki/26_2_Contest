...
from mvp_recommendation.bpr import BPRRecommender
...
class KnownUserRecommendationPipeline:
    ...
    @torch.no_grad()
    def _user_scores(self, user_id: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def recommend(
        ...
        unknown_models = set(requested_models) - set(MODEL_NAMES)
        if unknown_models:
        ...
        for user_id in map(int, user_ids):
            mf_z, text_z, available = self._user_scores(user_id)
